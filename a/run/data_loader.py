...
def prepare_inputs(x_train, x_test=None):
  ...
  if x_test is not None:
    ...
    x_train_features = Normalizer.transform(x_train_features)
    x_test_features = Normalizer.transform(x_test_features)
    return x_train_features, x_test_features

  x_features = pd.concat(train_features, axis=1).values
  Normalizer = MinMaxScaler((-1, 1))
