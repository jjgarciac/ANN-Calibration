...
jem_root = 'E:/UNC-CS-Course/COMP 790-142/projects/code/ANN-Calibration/experiments_buffer/results.txt'
df_result = pd.read_csv(jem_root, header=0)
model = df_result.values[:, 0]
datasets = df_result.values[:, 1]
id = df_result.values[:, 2]
acc = df_result.values[:, 3].astype('float')
ece = df_result.values[:, 4].astype('float')
oe = df_result.values[:, 5].astype('float')
auc = df_result.values[:, -1].astype('float')

###########################################
for i_d in datasets:
    current_dataset = i_d.replace(' ', '')
    dict_results['ACC'][current_dataset] = {}
    dict_results['ECE'][current_dataset] = {}
    dict_results['OE'][current_dataset] = {}
    dict_results['AUC'][current_dataset] = {}
for i in range(df_result.values.shape[0]):
    ...
    dict_results['AUC'][current_dataset][model[i]] = auc[i]
#for i_D in DATASETS:
#    dict_results[i_D]['ACC'] = pd.Series(dict_results[i_D]['ACC']).to_string()
#    dict_results[i_D]['ECE'] = pd.Series(dict_results[i_D]['ECE']).to_string()
#    dict_results[i_D]['OE'] = pd.Series(dict_results[i_D]['OE']).to_string()
...
for i_d in datasets:
    current_dataset = i_d.replace(' ', '')
    list_jehmo_acc.append(dict_results['ACC'][current_dataset]['jehmo'] - dict_results['ACC'][current_dataset]['ann'])
    list_jehmo_ece.append(dict_results['ECE'][current_dataset]['jehmo']-dict_results['ECE'][current_dataset]['ann'])
    list_jehmo_oe.append(dict_results['OE'][current_dataset]['jehmo'] - dict_results['OE'][current_dataset]['ann'])
    list_jehmo_auc.append(dict_results['AUC'][current_dataset]['jehmo'] - dict_results['AUC'][current_dataset]['ann'])

ACC_m = np.around(np.array(list_jehmo_acc).mean(), 3)
AUC_m = np.around(np.array(list_jehmo_auc).mean(), 3)
ECE_m = np.around(np.array(list_jehmo_ece).mean(), 3)
OE_m = np.around(np.array(list_jehmo_oe).mean(), 3)

current_metrics = {}
for i_metrics in ['ACC', 'ECE', 'OE', 'AUC']:
    current_metrics[i_metrics] = {}
    for i_model in ['jehmo']:
        current_metrics[i_metrics][i_model] = []
        for i_dataset in datasets:
            i_dataset = i_dataset.replace(' ', '')
            current_improve = dict_results[i_metrics][i_dataset][i_model] - dict_results[i_metrics][i_dataset]['ann']
...
pd.DataFrame(current_metrics).to_latex('../results/average_improvement_jehmo', multirow=True)
